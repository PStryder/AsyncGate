"""AsyncGate's binding to the shared MetaGate bootstrap client.

The client lives in the LegiVellum shared package so the gates do not each grow
their own copy -- the four identical `parents[4]` IndexErrors fixed across four
repos are what duplicated integration code looks like a few months on. All this
module contributes is the one genuinely AsyncGate-specific thing: which
primitive types map to which settings.
...
# AsyncGate emits receipts, so ReceiptGate is the peer it needs resolved.
BOOTSTRAP_BINDINGS = (
    (_shared.EndpointBinding(primitive_type="receiptgate", setting="receiptgate_endpoint"),)
...
async def bootstrap_from_metagate(settings: Any) -> Any:
    """Resolve AsyncGate's peer endpoints from MetaGate."""
    if _shared is None:
        logger.warning(
        ...
        return None
    return await _shared.bootstrap_from_metagate(settings, bindings=BOOTSTRAP_BINDINGS)
